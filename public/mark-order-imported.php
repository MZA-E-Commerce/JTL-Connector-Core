<?php
// http://jtl-connector-pimcore.mza.de/mark-order-imported.php?
// token=YBUb89fQV1j0Apa1fFLtEDSyXnHRlopk
// &externeAuftragsNr={{ Vorgang.Stammdaten.ExterneAuftragsnummer }}
// &auftragsNr={{ Vorgang.Stammdaten.Auftragsnummer }}
// &auftragsId={{ Vorgang.Stammdaten.InterneAuftragsnummer }}&reset=0
 ...
 * @throws Exception
 */
function markOrderAsImported($pimcoreOrderId, $jtlOrderNumber, $jtlOrderId, bool $reset = false)
{
    ...
    $apiUrl = $apiBaseUrl . '?orderId=' . urlencode($pimcoreOrderId) . '&jtlOrderId=' . urlencode($jtlOrderId) . '&jtlOrderNumber=' . urlencode($jtlOrderNumber);

    // URL: Replace orderId
    if ($reset) {
        $apiUrl.= '&reset=1';
    }
    logMessage("Calling Pimcore-API: $apiUrl");
...
try {
    ...
    // Get order data
    $jtlOrderNumber = $_GET['auftragsNr'] ?? null;
    $jtlOrderId = $_GET['auftragsId'] ?? null;
    $pimcoreOrderId = $_GET['externeAuftragsNr'] ?? null;

    $reset = isset($_GET['reset']) && $_GET['reset'];

    if (!$jtlOrderId) {
        http_response_code(400);
        die('Missing "auftragsId"');
    }
    if (!$jtlOrderNumber) {
        http_response_code(400);
    ...
    if (!$pimcoreOrderId) {
        http_response_code(400);
        die('Missing "externeAuftragsNr"');
    }

    if ($reset) {
        logMessage("Resetting order: externeAuftragsNr=$pimcoreOrderId");
    } else {
        logMessage("Processing order: externeAuftragsNr=$pimcoreOrderId");
    }

    // Call ESB API
    $apiResponse = markOrderAsImported($pimcoreOrderId, $jtlOrderNumber, $jtlOrderId, $reset);

    // Optional: Order in processed_orders.json speichern
    saveProcessedOrder($pimcoreOrderId);
